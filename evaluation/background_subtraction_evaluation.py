...
def contains_relevant_files(root):
    return os.path.isdir(root + '/' + 'groundtruth') and os.path.isdir(root + '/' + 'input')


def find_relevant_dirs(root):
    relevant_dirs = []
    for d in sorted(os.listdir(root)):
        if d == 'groundtruth' or d == 'input':        
            d2 = root + '/' + d
            if os.path.isdir(d2):
                relevant_dirs += [root]
...
def load_sequence(root):
    gt_dir, frames_dir = root + '/' + 'groundtruth', root + '/' + 'input'
    gt = get_files_in_dir('png', path = gt_dir, show_folders = False, fullpath = True)
    f = get_files_in_dir('jpg', path = frames_dir, show_folders = False, fullpath = True)
...
def evaluate_algorithm(gt, frames, algo, algo_name, algo_arguments):
...
#             gt_mask_image = Image.fromarray(gt_mask)
    ...
    for idx, prediction_mask in enumerate(mask[2450:2475]):
        pred_mask_image = Image.fromarray(prediction_mask)
        pred_mask_image.save(f'images/{algo_name}_pred_mask_image_{idx}.jpg')
...
def evaluate_on_sequence(seq, summary):
    ...
    for algo, algo_name, algo_arguments in ALGORITHMS_TO_EVALUATE:
        ...
        sec_per_step, precision, recall, f1, accuracy = evaluate_algorithm(gt, frames, algo, algo_name, algo_arguments)
